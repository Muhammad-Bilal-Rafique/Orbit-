...
const ProductsList = ({ Products }: { Products: ProductTypes[] }) => {
  ...
  const handleRefetch = async () => {
    try {
      const res = await fetch("/api/user/getAllProducts", {
        cache: "no-store",
      });
      ...
      console.error(" Client runtime refetch failed:", error);
    }
  };
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
          {products.map((product, index) => {
            ...
            const colors =
              product.attributes?.find((a) => a.name === "Color")?.values || [];
            const sizes =
              product.attributes?.find((a) => a.name === "Size")?.values || [];
              ...
              >
                ...
                <div className="flex items-center gap-4 w-70 shrink-0">
                  ...
                  <div className="truncate">
                    <p className="font-medium text-foreground tracking-tight truncate">
                      {product.name}
                    </p>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <div className="flex gap-1 flex-wrap">
                        {colors.map((c) => (
                          <span
                            key={c}
                            className="text-[10px] px-2 py-0.5 rounded-sm bg-secondary border border-border text-foreground font-medium"
                          >
                            {c}
                          </span>
                        ))}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <div className="flex gap-1 flex-wrap">
                        {sizes.map((s) => (
                          <span
                            key={s}
                            className="text-[10px] px-1.5 py-0.5 rounded-sm bg-secondary border border-border text-foreground font-bold"
                          >
                            {s}
                          </span>
                        ))}
                ...
                <div className="flex items-center gap-8 shrink-0">
                  <div className="text-right w-32">
                    <p
                      className={`text-sm font-semibold ${totalStock > 10 ? "text-emerald-500" : totalStock > 0 ? "text-amber-500" : "text-destructive"}`}
                    >
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="tracking-tight">
              Modify Inventory Configurations
            </DialogTitle>
          </DialogHeader>
          {selectedProduct && (
            <EditInventoryItem
