...
export default function OrderHistoryCard({ order }: OrderHistoryCardProps) {
  ...
  const getStatusBlock = (status: OrderStatus) => {
    switch (status) {
      case "pending":
        return (
          <Badge className="bg-blue-500/10 text-blue-500 border-blue-500/20 shadow-none">
            Pending
          </Badge>
        );
      case "processing":
        return (
          <Badge className="bg-amber-500/10 text-amber-500 border-amber-500/20 shadow-none">
            Processing
          </Badge>
        );
      case "delivered":
      ...
      <CardContent className="p-4 md:p-6 space-y-4">
        ...
        <div className="space-y-4">
          {order.items.map((item) => (
            <div
              key={item._id}
              className="flex flex-col sm:flex-row sm:items-center justify-between text-sm py-2 gap-2 border-b border-border/40 last:border-0 pb-3 last:pb-0"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Quantity Vector: {item.quantity} Pcs
                  </p>
                </div>
              </div>
