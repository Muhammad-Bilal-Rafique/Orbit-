...
export default function AddProduct() {
  ...
  useEffect(() => {
    if (colors.length > 0 && sizes.length > 0) {
      const generatedMatrix = colors.flatMap((color) =>
        sizes.map((size) => {
          const old = variants.find((v) => v.color === color && v.size === size);
          return {
            color,
            size,
            price: old ? old.price : "",
  ...
  const onSubmit: SubmitHandler<ProductAdminTypes> = async (data) => {
    try {
      ...
      console.error("Critical dashboard action configuration fail:", error);
    }
  };
    ...
    <div className="min-h-screen bg-background text-foreground py-12 px-4 sm:px-6 lg:px-8 font-sans antialiased">
      ...
      >
        ...
        <div className="border-b border-border pb-6 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Register global specifications, assets, and size-color matrix stock arrays.</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Product Identity / Name</Label>
            <Input 
            ...
            />
            {errors.name && <p className="text-destructive text-xs font-medium">Product name string mapping required.</p>}
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            {errors.description && <p className="text-destructive text-xs font-medium">Description context stream cannot remain null.</p>}
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Base Retail Price ($)</Label>
              ...
              {errors.price && <p className="text-destructive text-xs font-medium">Price configuration node fault.</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.category && <p className="text-destructive text-xs font-medium">Category placement matrix target mandatory.</p>}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5 border border-border p-6 rounded-md bg-secondary/30">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-primary border-b border-border pb-3">
            <Layers className="w-3.5 h-3.5" /> Dimensions & Variant Arrays Matrix
          </div>
          <div className="space-y-6 pt-1">
            ...
            {variants.length > 0 && (
              <div className="pt-2 animate-in fade-in duration-200">
                <VariantsMatrixTable variants={variants} setVariants={setVariants} />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Tags (For User to Search for)</Label>
            ...
            />
            {errors.keywords && <p className="text-destructive text-xs font-medium">Search query vectors declaration mandatory.</p>}
          </div>

          {/* STARK STYLED ASSET BOX */}
          <div className="space-y-2">
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Visual Asset Payload</Label>
            <div className="relative border border-dashed border-border hover:border-primary bg-background rounded-md p-6 transition-all group flex flex-col items-center justify-center gap-2 cursor-pointer text-center">
              ...
              <div className="space-y-0.5">
                <p className="text-xs font-medium text-foreground">Upload file binary stream</p>
                <p className="text-[11px] text-muted-foreground">JPG, PNG, or high-density WEBP source parameters</p>
              </div>
            </div>
            ...
            {imageUrl && (
              <div className="relative mt-3 w-24 h-24 border border-border rounded-md p-1 bg-background group animate-in zoom-in-95 duration-200">
                <img src={imageUrl} alt="preview" className="w-full h-full rounded-sm object-cover  transition-all" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border border-border bg-background p-4 rounded-md">
          <div className="space-y-0.5">
            ...
            <p className="text-[11px] text-muted-foreground">Add this product in the featured section.</p>
          </div>
          <Controller 
            name="isFeatured" 
            control={control} 
        ...
        <Button 
          type="submit" 
          className="w-full h-11 bg-primary text-primary-foreground hover:bg-primary/90 font-medium text-sm rounded-md tracking-wider transition-colors border border-transparent active:scale-[0.995]"
        >
          Publish Product
        </Button>
      </form>
