...
import { ShoppingCart, ArrowLeft, Trash2, ShieldCheck, Truck } from "lucide-react";
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center overflow-hidden">
        <MotionView stagger={0.1} className="max-w-md w-full mx-auto px-6 text-center flex flex-col items-center">
          <MotionItem>
            <div className="w-20 h-20 rounded-full bg-secondary/50 border border-border/60 flex items-center justify-center mb-6 shadow-xs relative">
              <ShoppingCart className="w-8 h-8 text-muted-foreground" strokeWidth={1.5} />
            </div>
          </MotionItem>
          ...
          <MotionItem>
            <p className="text-sm text-muted-foreground mb-8 max-w-xs mx-auto">
              Explore our premium minimalist apparel drops to add items to your inventory space.
            </p>
          </MotionItem>
          
          <MotionItem className="w-full">
            <Button size="lg" className="w-full rounded-full font-semibold cursor-pointer tracking-wide hover:scale-105 transition-all duration-300">
          ...
          <div className="lg:col-span-2 space-y-4 w-full">
            <div className="flex justify-between items-center pb-4 border-b border-border/40">
              <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
                Inventory space ({items.length} item{items.length !== 1 ? "s" : ""})
              </span>
              <button
                onClick={clearCart}
                ...
                {items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <CartItemComponent
                        item={item}
                        onUpdateQuantity={(id, col, sz, q) => updateQuantity(id, col, sz, q)}
              ...
              <div className="bg-card/40 border border-border/50 rounded-2xl p-6 lg:p-8 h-fit sticky top-24 shadow-xs backdrop-blur-xs">
                ...
                <div className="space-y-4 mb-6 pb-6 border-b border-border/40 text-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground/80 font-medium">Subtotal</span>
                    <span className="text-foreground font-semibold">${total.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground/80 font-medium">Tax (10%)</span>
                    <span className="text-foreground font-semibold">${tax.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground/80 font-medium">Shipping Fee</span>
                    <span className="font-semibold">
                      {shipping === 0 ? (
                        <span className="text-emerald-500 font-bold uppercase text-xs tracking-wider bg-emerald-500/5 px-2.5 py-1 border border-emerald-500/10 rounded-full">Free</span>
                      ) : (
                ...
                {shipping > 0 && (
                  <div className="mb-6 p-3 bg-secondary/30 border border-border/40 rounded-xl flex items-center gap-2 text-xs font-medium text-muted-foreground">
                    <Truck className="w-4 h-4 shrink-0 text-primary/70" strokeWidth={1.5} />
                    <span>Add <strong className="text-foreground">${(50 - total).toFixed(2)}</strong> more to unlock free delivery.</span>
                  </div>
                )}

                {/* Grand Total Value Anchor Row */}
                <div className="flex justify-between items-baseline mb-8">
                  <span className="text-sm font-bold uppercase tracking-wider text-foreground/70">Total Due</span>
                ...
                <div className="space-y-3 w-full">
                  <Button size="lg" className="w-full h-12 rounded-xl text-xs uppercase font-medium tracking-widest transition-all hover:scale-[1.01] cursor-pointer shadow-md shadow-primary/5">
                    <Link href="/users/checkout">Proceed to Checkout</Link>
                  </Button>
                  
                  <Button  size="lg" variant="outline" className="w-full h-12 rounded-xl text-xs uppercase font-medium tracking-widest bg-background/50 hover:bg-muted cursor-pointer transition-colors">
                    <Link href="/users/products">Continue Browsing</Link>
                  </Button>
                </div>
